...
a value from the distribution given by $nds$ (denoted $P(x \leq nds)$) is
just the sum of probabilities below and equal to x.
$$
P(x \leq nds) \doteq \sum^{x}_{k=0} P(k = nds)
$$
...
cdfPlot <- \(n,s) (
  ggplot()
    ...
    +theme_bw()
    +xlab(TeX("$x$"))
    +ylab(TeX(paste("$P(x \\leq ", as.character(n), "d", as.character(s), ")$", sep="")))
    +scale_x_continuous(breaks=0:(n*(s+1)))
)
